...
from multi_lake_dataset import (
    LakeData,
    load_one_lake,
    compute_global_norms,
    apply_norms_inplace,
    MultiLakeWindowDataset,
)
...
def parse_int_tuple3(x: str) -> Tuple[int, int, int]:
    parts = [int(t.strip()) for t in str(x).split(",")]
    if len(parts) != 3:
        raise ValueError("split_years must be 'train_end,val_end,max' e.g. 2018,2021,2025")
...
# -------------------------
# Lake loading + universal norms
# -------------------------
def load_lakes_by_name(root_dir: str, names: List[str]) -> List[LakeData]:
    lakes: List[LakeData] = []
    for nm in names:
...
def index_lakes(lakes: List[LakeData]) -> Dict[str, LakeData]:
    return {lk.name: lk for lk in lakes}
...
def main():
    ...
    # Back-compat: if you only provide lake_names, it will behave like before.
    ap.add_argument("--lake_names", type=str, default="BearLake,RedPond",
                    help="Comma-separated lake directory names inside root_dir (legacy: used for all roles)")

    # New: explicit role-based lake lists
    ap.add_argument("--norm_lakes", type=str, default=None,
                    help="Comma-separated lake names used to compute universal normalization (train years only).")
    ap.add_argument("--train_lakes", type=str, default=None,
                    help="Comma-separated lake names used for train/val and checkpoint selection.")
    ap.add_argument("--test_lakes", type=str, default=None,
                    help="Comma-separated lake names to run test metrics on after training.")
    ...
    # Resolve lake lists
    legacy_all = parse_csv_list(args.lake_names)

    norm_lakes = parse_csv_list(args.norm_lakes) if args.norm_lakes else legacy_all
    train_lakes = parse_csv_list(args.train_lakes) if args.train_lakes else legacy_all
    test_lakes = parse_csv_list(args.test_lakes) if args.test_lakes else legacy_all

    # Load the union of all lakes we will need (norm/train/test)
    union_names = sorted(set(norm_lakes) | set(train_lakes) | set(test_lakes))
    if len(union_names) == 0:
        raise ValueError("No lakes specified. Provide lake_names or norm_lakes/train_lakes/test_lakes.")
    ...
    # For testing we’ll evaluate per-lake (cleaner attribution)
    # but we still want dims. Use train_ds as reference.
    Du = train_ds.Du
    P = train_ds.P
    Dd = train_ds.Dd
    ...
    with mlflow.start_run(run_name=run_name):
        ...
        # Helpful tags
        mlflow.set_tag("norm_lakes", ",".join(norm_lakes))
        mlflow.set_tag("train_lakes", ",".join(train_lakes))
        mlflow.set_tag("test_lakes", ",".join(test_lakes))

        # log params
        mlflow.log_params({
            "root_dir": args.root_dir,
            "lake_names_union": ",".join(union_names),
            "norm_lakes": ",".join(norm_lakes),
        ...
        # training loop (val only on train_lakes)
        for ep in range(1, args.epochs + 1):
            ...
            if va < best_val:
                ...
                torch.save({
                    ...
                    "norms": norms,  # convenient for plotting scripts
                    "train_lakes": train_lakes,
                    "test_lakes": test_lakes,
                    "norm_lakes": norm_lakes,
                }, best_path)
                mlflow.log_artifact(best_path, artifact_path="checkpoints")

        # ---------------------------------------------------------
        # Testing: evaluate per-lake (test split), log separately
        # ---------------------------------------------------------
        test_metrics = {}
        for nm in test_lakes:
            lk = lakes_by_name[nm]
            # make a dataset with ONLY this lake to attribute results cleanly
            ...
            mse = eval_epoch(loader, model, args.device)
            test_metrics[nm] = float(mse)
            mlflow.log_metric(f"test_mse__{nm}", float(mse))
            print(f"[TEST] {nm}: MSE={mse:.6f}")

        # Optional aggregate: mean over lakes
        if len(test_metrics) > 0:
            mean_test = float(np.mean(list(test_metrics.values())))
            mlflow.log_metric("test_mse__mean_over_test_lakes", mean_test)
        ...
        # also save a small json summary of per-lake test results
        test_path = os.path.join(args.artifacts, "test_metrics.json")
        with open(test_path, "w") as f:
            json.dump(test_metrics, f, indent=2)
        mlflow.log_artifact(test_path, artifact_path="artifacts")

        print(f"Best val MSE (train_lakes only): {best_val:.6f}")
        print(f"Per-lake test metrics: {test_metrics}")
